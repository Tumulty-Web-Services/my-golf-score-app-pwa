...
exports.default = exports.filterPresetsConfig = void 0;
...
var filterPresetsConfig = function (presetsConfig) {
  return presetsConfig.filter(function (preset) {
    return !/@storybook[\\\\/]preset-typescript/.test(preset.name || preset);
  });
};
...
var _default = async function _default(options) {
  ...
  var presetsConfig = [...corePresets, require.resolve('./common/babel-cache-preset.js'), ...frameworkPresets, ...(0, _customPresets.default)(options), ...overridePresets]; // Remove `@storybook/preset-typescript` and add a warning if in use.
