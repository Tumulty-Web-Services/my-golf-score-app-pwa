...
function genStoryExport(ast, context) {
  ...
  if (!bodyNodes.length) {
    ...
    var storyReactCode = bodyParts.length > 1 ? "<>\n".concat(storyCode, "\n</>") : storyCode; // keep track if an indentifier or function call
...
function genCanvasExports(ast, context) {
  ...
  for (var i = 0; i < ast.children.length; i += 1) {
    ...
    if (child.type === 'JSXElement' && child.openingElement.name.name === 'Story') {
      ...
      var _generate8 = generate(child, {}),
          code = _generate8.code;
  ...
  if (title) {
    if (title.type === 'StringLiteral') {
      ...
      try {
        ...
        var _generate9 = generate(title, {}),
            code = _generate9.code; // remove the curly brackets at start and end of code
...
function getExports(node, counter, options) {
  ...
  if (type === 'jsx') {
    if (STORY_REGEX.exec(value)) {
      ...
      var _generate10 = generate(ast, {}),
          code = _generate10.code; // eslint-disable-next-line no-param-reassign
    ...
    if (CANVAS_REGEX.exec(value)) {
      ...
      var _generate11 = generate(_ast, {}),
          _code = _generate11.code; // eslint-disable-next-line no-param-reassign
...
function extractExports(root, options) {
  ...
  root.children.forEach(function (child) {
    if (child.type === 'jsx') {
      try {
        ...
        var _generate12 = generate(ast, {}),
            code = _generate12.code; // eslint-disable-next-line no-param-reassign
  ...
  var defaultJsx = mdxToJsx.toJSX(root, {}, Object.assign(Object.assign({}, options), {}, {
    skipExport: true
  }));
