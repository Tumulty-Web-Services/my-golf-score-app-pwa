...
var getSnippet = function getSnippet(storyId, sourceContext, docsContext) {
  ...
  var snippet = sources && sources[storyId];
  var data = storyStore === null || storyStore === void 0 ? void 0 : storyStore.fromId(storyId);

  if (!data) {
    // Fallback if we can't get the story data for this story
    _clientLogger.logger.warn("Unable to find source for story ID '".concat(storyId, "'"));

    return snippet || '';
  }

  var parameters = data.parameters; // eslint-disable-next-line no-underscore-dangle
  ...
  if (userCode) return userCode; // if user has explicitly set this as dynamic, use snippet
  ...
  var enhanced = (0, _enhanceSource.enhanceSource)(data) || data.parameters;
...
var getSourceProps = function getSourceProps(props, docsContext, sourceContext) {
  ...
  if (!source) {
    ...
    source = targetIds.map(function (storyId) {
      return getSnippet(storyId, sourceContext, docsContext);
    }).join('\n\n');
  }
